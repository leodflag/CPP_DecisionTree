...
typedef struct Node{
	int col;  // 資料所在行數 
	string NodeData;  // 資料欄位名稱
	struct goalData left;  // 左內部屬性 
	struct goalData middle; // 中內部屬性
	struct goalData right;  // 右內部屬性
...
Data_Str make_goal_data(string GoalStr,int c){  //小表 ：目標字，所在欄位
	int data_row=15;
	int data_col=6;
	Data_Str Data;
...
int main() {
	...
	string *goalArray=returnGoalArray(MaxNode.col); // 找最大資訊獲利欄的字串陣列
	...
	GO(goalArray,MaxNode.col);  // 輸入最大資訊獲利欄的字串陣列以建造樹 
	inOrder(head);  // 中序走訪 
	return 0;
}
